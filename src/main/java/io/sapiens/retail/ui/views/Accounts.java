...
import io.sapiens.retail.ui.components.Badge;
...
import io.sapiens.retail.ui.util.*;
import io.sapiens.retail.ui.util.css.*;
import io.sapiens.retail.ui.util.css.lumo.BadgeColor;
import io.sapiens.retail.ui.util.css.lumo.BadgeShape;
import io.sapiens.retail.ui.util.css.lumo.BadgeSize;
...
public class Accounts extends ViewFrame {
  ...
  private Grid createGrid() {
    ...
    // "Mobile" column
    grid.addColumn(new ComponentRenderer<>(this::getMobileTemplate)).setVisible(false);
    ...
    grid.addColumn(
            new LocalDateRenderer<>(
                BankAccount::getUpdated, DateTimeFormatter.ofPattern("MMM dd, YYYY")))
        .setAutoWidth(true)
  ...
  private class BankAccountMobileTemplate extends FlexBoxLayout {
    ...
    private FlexBoxLayout getOwner() {
      ...
      Badge id =
          new Badge(
              String.valueOf(bankAccount.getId()), BadgeColor.NORMAL, BadgeSize.S, BadgeShape.PILL);
    ...
    private Label getAccount() {
      Label account =
          UIUtils.createLabel(FontSize.S, TextColor.SECONDARY, bankAccount.getAccount());
    ...
    private FlexBoxLayout getAvailability() {
      ...
      Label updated =
          UIUtils.createLabel(
              FontSize.XS, TextColor.TERTIARY, UIUtils.formatDate(bankAccount.getUpdated()));
