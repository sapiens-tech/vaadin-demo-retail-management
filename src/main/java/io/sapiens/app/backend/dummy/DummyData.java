package io.sapiens.app.backend.dummy;
...
//      person.setDateOfBirth(getDate());
    ...
    for (i = 0; i < 40; i++) {
      BANK_ACCOUNTS.put(
          ...
          new BankAccount(
              ...
              getIBAN(),
              getCompany(),
              getRandomDouble(5000, 100000),
              getDate(),
              DummyData.getImageSource()));
    }
    ...
    HEALTHCARE.put(
        "Vicks VapoRub", "Vick® Vaporub makes a mother's touch a touch more comforting.");
    HEALTHCARE.put(
    ...
    DENTAL.put(
        "Syringe", "A dental syringe is a used by dentists for the injection of an anesthetic.");
    DENTAL.put(
        "Drill",
    ...
    CONSTRUCTION.put(
        "Safety Helmet", "UV-resistant high density Polyethylene (HDPE) safety helmet.");
    ...
    CONSTRUCTION.put(
        "Metal Dump Truck", "2mm thick steel sheet with double folded top rim construction.");
    CONSTRUCTION.put(
        "Work Platform",
        "Aluminium 'Hop-Up' with unique folding first step to provide extra working level.");
    CONSTRUCTION.put(
        "Scaffold Systems", "All aluminium construction with timber platform 1300 x 460mm.");
  ...
  public static Invoice.Status getInvoiceStatus() {
    return Invoice.Status.values()[random.nextInt(Invoice.Status.values().length)];
  }
